...
/**
 * Apple VideoToolbox encoding strategy
 */
export class AppleVideoToolboxEncodingStrategy implements EncodingStrategy {
  ...
  async isAvailable(): Promise<boolean> {
    return new Promise(resolve => {
      ...
      ffmpeg()
        ...
        .output('-')
        .on('error', () => {
          logger.debug('Apple VideoToolbox not available');
          resolve(false);
        })
        .on('end', () => {
          logger.debug('Apple VideoToolbox is available');
          resolve(true);
        })
        .run();
    });
